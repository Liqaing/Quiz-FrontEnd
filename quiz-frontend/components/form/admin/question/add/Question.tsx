    ...
    async function getQuestion() {
        if (props.questionId) {
            ...
            if (data) {                
                ...
                data.answers.forEach((answer:Answer, index:any) => {
                    ...
                    <form className="p-4 md:p-5" action={props.formAction}>
                        ...
                        <div className="grid gap-4 mb-4 grid-cols-4">
                            ...
                            <div className="col-span-4">
                                ...
                                <select id="type" name="type" value={type} onChange={e => setQuestion(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            ...
                            {answers.map((ans, index) => (
                                <div className="col-span-2" key={index}>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" hidden name={`answerId${index}`} id={`answerId${index}`} value={ans. ? props.questionId : ""} readOnly/>
                                        
                                        <input type="text" id={`answer${index}`} name={`answer${index}`} value={ans} onChange={e => handleAnswerChange(index, e.target.value)} className="block w-full p-3 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder={`Enter answer ${index + 1}`} required />
                                        <input type="radio" name="correctAnswer" id={`correctAnswer${index}`} value={index} checked={correctAnswer === answers[index]} onChange={() => handleCorrectAnswerChange(index)} className="text-white absolute end-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" />
                                    </div>                                
                                </div>
                            ))}
